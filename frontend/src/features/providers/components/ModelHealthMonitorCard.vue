      ...
      >
        <div>
          ...
          <p class="mt-1 text-xs text-muted-foreground">
            基于真实请求统计模型可用率、平均耗时、平均TTFB 与平均速度
          </p>
        </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        >
          ...
          <div class="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
            ...
            >
              <div
                class="flex items-center gap-1.5 text-[11px] leading-tight text-muted-foreground"
              >
...
import { Activity, Bot, Gauge, Loader2, Radio, Zap } from "lucide-vue-next";
...
function buildTimelineTooltip(
  ...
  const startMs = monitor.time_range_start
    ? new Date(monitor.time_range_start).getTime()
    : Date.now() - parseInt(lookbackHours.value) * 60 * 60 * 1000;
  const endMs = monitor.time_range_end
    ? new Date(monitor.time_range_end).getTime()
    : Date.now();
