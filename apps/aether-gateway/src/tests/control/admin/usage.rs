...
#[tokio::test]
async fn internal_retry_usage_filter_and_detail_include_previous_candidate_retries() {
    let mut item = sample_usage_row(
        "usage-internal-retry",
        "req-internal-retry",
        Some("user-1"),
        Some("key-1"),
        Some("primary"),
    ...
    item.candidate_id = Some("cand-internal-final".into());
    item.candidate_index = Some(1);
    let audit = |count| {
        json!({"version":1,"scope":"aether","kind":"same_plan_overload","retry_count":count,
        "failures":[],"stop_reason":"budget_exhausted"})
    };
    item.request_metadata = Some(json!({"internal_retry":audit(0)}));
    let mut first = sample_request_candidate(
        "cand-internal-first",
        "req-internal-retry",
        0,
        0,
    ...
    first.extra_data = Some(json!({"internal_retry":audit(2)}));
    let mut last = sample_request_candidate(
        "cand-internal-final",
        "req-internal-retry",
        1,
        0,
    ...
    last.extra_data = Some(json!({"internal_retry":audit(0)}));
    let usage = Arc::new(InMemoryUsageReadRepository::seed(vec![item]));
    let candidates = Arc::new(InMemoryRequestCandidateRepository::seed(vec![first, last]));
    let gateway = build_router_with_state(AppState::new().unwrap().with_data_state_for_tests(
        GatewayDataState::with_request_candidate_and_usage_repository_for_tests(candidates, usage),
    ));
    let (url, handle) = start_server(gateway).await;
    let response = admin_request(reqwest::Client::new().get(format!(
        "{url}/api/admin/usage/records?start_date=2024-03-21&end_date=2024-03-22&tz_offset_minutes=0&status=has_retry&limit=10&offset=0")))
        .send().await.unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    let body: serde_json::Value = response.json().await.unwrap();
    assert_eq!(body["records"].as_array().unwrap().len(), 1);
    let record = &body["records"][0];
    assert_eq!(record["has_retry"], true);
    assert_eq!(record["internal_retry"]["retry_count"], 2);
    assert_eq!(record["internal_retry"]["complete"], true);
    assert_eq!(record["internal_retry"]["outcome"], "succeeded");
    let response = admin_request(reqwest::Client::new().get(format!(
        "{url}/api/admin/usage/usage-internal-retry?include_bodies=false"
    )))
    .send()
    .await
    .unwrap();
    ...
    assert_eq!(detail["internal_retry"], record["internal_retry"]);
    assert_eq!(detail["has_retry"], true);
    handle.abort();
}
