    ...
    fn shrink_empty_out_buffer(&mut self) {
            ...
            && self.out_buffer.capacity() > self.max_retained_out_buffer_capacity
        {
            let retained_capacity =
                self.out_buffer_write_len.min(self.max_retained_out_buffer_capacity);
...
mod tests {
    ...
    impl PartialWouldBlockWriter {
        fn new(first_write_limit: usize) -> Self {
            Self { bytes: Vec::new(), first_write_limit, write_calls: 0, blocked: false }
        }
    }
    ...
    #[test]
    fn large_terminal_frame_releases_oversized_write_buffer_capacity() {
        const WRITE_BUFFER_SIZE: usize = 128 * 1024;
        const MAX_RETAINED_CAPACITY: usize = 256 * 1024;
        let mut terminal = vec![b'x'; 16 * 1024 * 1024];
        let prefix = br#"{"type":"response.completed"}"#;
        terminal[..prefix.len()].copy_from_slice(prefix);
        let frame = Frame::message(terminal, OpCode::Data(Data::Text), true);
        let expected_len = frame.len();
        let mut codec = FrameCodec::new(1024);
        codec.set_max_out_buffer_len(17 * 1024 * 1024);
        codec.set_out_buffer_write_len(WRITE_BUFFER_SIZE);
        codec.set_max_retained_out_buffer_capacity(MAX_RETAINED_CAPACITY);
        let mut writer = Vec::new();

        codec.buffer_frame(&mut writer, frame).expect("terminal frame should drain");
    ...
    fn partial_would_block_keeps_bytes_and_shrinks_only_after_retry_drains() {
        ...
        frame.clone().format_into_buf(&mut expected).expect("frame should format");
        let mut codec = FrameCodec::new(1024);
        ...
        codec.write_out_buffer(&mut writer).expect("retry should drain queued bytes");
