...
### 18.22 `.133`：Responses Lite 能力跟随官方 manifest（运行时拉取 + 内置兜底 + 漂移观测），WS 步骤体标记与目标模型一致

**起因**：`.127` 起按静态名单加 `x-openai-internal-codex-responses-lite`，`.130` 首次带上线后连出两个 400 签名（有头没体：第三方客户端发 sol；头在非 lite 模型：号池把 luna 重映射到 gpt-5.5），`.131` / `.132` 两次热修把规则收敛到「lite body ∧ lite **目标**模型」。规则对了，但名单 `RESPONSES_LITE_MODELS` 仍是手抄 codex-rs `models-manager/models.json` 的 7 个 slug；官方每 2–4 天一个 minor，新 lite 模型出现或某模型 lite 被撤时，网关要等人发现、改常量、发版。操作员问「这份名单如何长期维护」，在三层方案里选了第三层，并把第一、二层作为兜底与观测一起并入。

**官方基准**（codex-rs `07f18d5f`）：`models-manager/src/manager.rs` 缓存 TTL 300 s，`fetch_and_update_models` 用 `client_version_to_whole()`（`major.minor.patch`）调 `codex-api/src/endpoint/models.rs::list_models` → `GET {base}/models?client_version=X`，普通 GET，只把响应 `ETag` 记下；每个 `/responses` 响应（HTTP SSE 与 WS）带 `X-Models-Etag`，`refresh_if_new_etag` 变了就重拉。能力字段 `protocol/src/openai_models.rs::ModelInfo.use_responses_lite`。服务端按 `client_version` 门控 `minimal_client_version`：模型同步硬编码的 `0.128.0-alpha.1` 只能看到 3 个模型（`codex-auto-review` / `gpt-5.3-codex-spark` / `gpt-5.5`），sol / terra / luna / astra / daybreak 全被隐藏，所以必须用被跟随的真实客户端版本去拉。WS 面：`core/src/client.rs` 在 `use_responses_lite` 时往每步 body 的 `client_metadata` 里放 `ws_request_header_x_openai_internal_codex_responses_lite = "true"`（`HashMap<String, String>`，是字符串不是布尔），后端校验它与 HTTP 头相同；`.132` 及之前 Aether 对这个键是入站透传（`FLAT_PASS_KEYS`），近 48h WS 上的 Codex 400 全部是 luna / terra / auto-review→gpt-5.5 的重映射请求，客户端随后回落 HTTP 得 200。

**修法**（新模块 `codex_model_catalog.rs` + `codex_model_catalog/bundled.json`；`crates/aether-model-fetch` 新增 `build_codex_models_manifest_execution_plan`）：

1. **第一层（兜底）**：`bundled.json` 是 `models.json` 的精简副本——11 条，只留 `slug` / `use_responses_lite` / `prefer_websockets` / `supported_in_api` / `visibility` / `minimal_client_version` / `priority`，不含任何 `base_instructions` 提示词（单测钉住「序列化结果不含 `instructions`」、11 个模型、恰好 7 个 lite slug）。`RESPONSES_LITE_MODELS` 常量删除。
2. **第三层（主体）**：后台任务 `codex.model_catalog.worker`（`supervise_worker` 托管，启动 15 s 后开始，每 300 s 一轮，`AETHER_CODEX_MODEL_CATALOG_INTERVAL_SECS` 可调、下限 60）。目标账号 = 活跃 Codex provider × `openai:responses` 且 base_url 为 `chatgpt.com/backend-api/codex` 的 endpoint × 活跃 key（忽略 `auto_fetch_models`），按 `key_id` 排序、按轮次轮转、一轮最多 3 次。被跟随版本 = 发版登记表（§18.21）所有 provider × family 的最大稳定版（新增 `ClientReleaseStore::families()` 扫 family），无记录退回 `CODEX_MANIFEST_FALLBACK_CLIENT_VERSION`。计划复用模型同步的 `build_standard_models_fetch_execution_plan`（bearer、`chatgpt-account-id`、代理、header rules 全在），只把 URL 的 `client_version` 换掉；账号有具体 profile 且冻结 UA 的版本 token 能换到被跟随版本时，UA / originator / `version` 头按该账号身份（`compose_codex_client_user_agent`）；冻结 UA 是预发布版（注册表只收稳定版，换不动）或没有 profile 时保持模型同步的 `openai-codex/1.0`、不带 `version` / `originator`——URL 的 `client_version` 与 UA 永远不各说一个版本。执行走 `execute_execution_runtime_sync_plan`（不参与 key 健康分）。2xx → 解析（无 slug 的条目跳过）→ Redis `aether:codex:model_catalog:v1:<version>`（TTL 7 天）→ 进程内快照（`parking_lot::RwLock<Option<Arc<..>>>`）；非 2xx / 传输错 → 换下一个账号，全失败保留旧快照。同版本文档在一个刷新间隔内新鲜时只装载不请求；`refresh_lock`（60 s）保证多实例只一个拉。启动先读 Redis（先按被跟随版本，再扫最新版本）。
3. **决策接线**：`codex_routing_hint::model_name_uses_responses_lite(model)` = 快照命中（true / false 都算）→ 内置副本 → false。`apply_responses_lite_header` 与四个 HTTP 位点、WS 候选头签名不变，新增 `_with(live, ..)` 纯函数供测试。语义：新 lite 模型 → 快照 true → 不发版即带头；某模型 lite 被撤 → 快照 false 覆盖内置 true；快照没列（版本门控 / 自定义名）→ 内置。
4. **WS `.133`**：`materialize_codex_ws_step_body` 在身份改写之后、env-context 改写之前调 `apply_codex_ws_responses_lite_flag(body, mapped_model)`：`client_metadata` 为对象时，该键 ⇔ `body_uses_responses_lite(body)` ∧ `model_name_uses_responses_lite(mapped_model)`，满足置字符串 `"true"`，不满足删键；没有 `client_metadata` 不创建。首步与增量步同规则。`report_context` 里的 `identity.responses_lite` 仍是入站观测值。
5. **第二层（观测）**：每次成功刷新比较快照与内置的 lite 集合：`added` / `removed` 非空 → `codex_model_catalog_lite_drift` warn（同签名一次），`hidden`（被版本门控藏起）只 debug。快照 24 h 没刷新成功 → 每小时 `codex_model_catalog_stale`。
6. **开关**：`AETHER_CODEX_MODEL_CATALOG=off` 不起任务、不读 Redis，只用内置副本（`.132` 行为）。

**不做**：不观测 `/responses` 响应头 `X-Models-Etag` 触发即时重拉（落点在 `execution_runtime/stream/execution.rs`，非本人文件、不 rustfmt-clean；300 s 定时足够）；不改模型同步的 `0.128.0-alpha.1`（会让 `allowed_models` 膨胀）；不做管理端手动刷新；不改写 body 契约（网关仍不把 plain body 造成 lite body）。

**测试**：`aether-model-fetch` 新增 `codex_manifest_plan_asks_with_the_followed_client_version`、`set_query_param_replaces_or_appends`（标准计划仍钉 `0.128.0-alpha.1`）。`codex_model_catalog::tests` 12 项：内置副本字段 / 数量 / 无提示词、manifest 解析白名单与跳过无 slug、大小写 / trim、Redis 往返 + 最新版本扫描 + TTL、被跟随版本取最大 / 回退 / `families()`、漂移分类与「同签名只 warn 一次」、header 大小写与 base64 body、目标账号筛选（非 chatgpt 后端与非 Codex provider 排除、inactive key 排除）、`refresh_once` 全流程（401 → 换账号 200、URL 带登记表版本、UA `openai-codex/1.0`、新鲜文档不请求、锁被占跳过、全失败保留旧快照、无 `models` 数组不装载）、无账号跳过 + 启动读回。`codex_routing_hint` 新增 live 快照优先级用例与 WS 标记用例；`codex_ws::runtime` 新增 `materialized_step_body_lite_flag_matches_the_body_and_the_target_model`（luna→luna 补 `"true"`；luna→gpt-5.5 删键、lite body 原样；plain body 删键；无 `client_metadata` 不创建）。

**交付状态**：提交到 `custom` 并打 tag `backend-v0.7.133`（CI 自动构建镜像）。**未上线**——`.132` 与 `.133` 的 `update.sh` 上线时机都由操作员另行决定；上线后复核见运维 runbook §3.13。
