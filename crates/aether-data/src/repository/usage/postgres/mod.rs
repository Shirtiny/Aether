...
FROM usage_routing_snapshots AS source_routing
...
WHERE source_routing.candidate_id = $1
  ...
  AND (
    ...
    OR source.api_key_id IS NOT DISTINCT FROM current.api_key_id
  )
...
SELECT
  source.request_id AS source_request_id,
  source.request_metadata->'prompt_capture' AS prompt_capture
FROM request_candidates AS source_candidate
...
fn push_postgres_usage_session_id_filter(
    builder: &mut QueryBuilder<'_, Postgres>,
    has_where: &mut bool,
    session_id: &str,
    exact: bool,
...
SELECT
  ...
  COALESCE(SUM(GREATEST(COALESCE("usage".effective_input_tokens, 0), 0)), 0)::BIGINT AS effective_input_tokens,
  ...
  COALESCE(SUM(GREATEST(COALESCE("usage".total_input_context, 0), 0)), 0)::BIGINT AS total_input_context,
            ...
            match row.kind.as_str() {
                ...
                USAGE_COUNTER_KIND_PROVIDER_API_KEY => {
                    ...
                    entry.total_response_time_ms = entry
                        ...
                        .saturating_add(row.total_response_time_ms_delta);
                    merge_optional_max(
                        &mut entry.candidate_last_used_at_unix_secs,
                        row.candidate_last_used_at_unix_secs,
                    );
