...
pub fn normalize_total_input_context_for_cache_hit_rate(
    ...
    let fresh_input_tokens = match parse_api_family(api_format) {
        ApiFamily::Claude => normalized_input_tokens,
        ...
        ApiFamily::Unknown => normalized_input_tokens,
    };
...
mod tests {
    ...
    fn subtracts_cache_tokens_for_openai_and_gemini() {
        assert_eq!(
            normalize_input_tokens_for_billing(Some("openai:chat"), 100, 15, 20),
            65
        );
        assert_eq!(
            normalize_input_tokens_for_billing(Some("gemini:generate_content"), 100, 15, 20),
            65
        );
    }
    ...
    fn keeps_input_tokens_for_claude() {
        assert_eq!(
            normalize_input_tokens_for_billing(Some("claude:messages"), 100, 15, 20),
            100
        );
    }
